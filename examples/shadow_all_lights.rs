...
impl AppHandler for ShadowAllLightsDemo {
    fn init(engine: &mut Engine, _window: &dyn Window) -> Self {
        ...
        scene
            ...
            .set_position(0.0, 3.0, 3.0);

        let point_helper = scene.spawn_sphere(
            0.1,
            PhysicalMaterial::new(Vec4::new(1.0, 0.8, 0.5, 1.0)).with_emissive(Vec3::new(1.0, 0.8, 0.5), 1.0),
            &engine.assets,
        );
    ...
    fn update(&mut self, engine: &mut Engine, window: &dyn Window, frame: &FrameState) {
        ...
        if let Some(fps) = self.fps_counter.update() {
            window.set_title(&format!("All Shadows (Dir + Spot + Point) | FPS: {:.2}", fps));
        }
    }
}
