...
impl ForwardRenderPass {
    ...
    fn prepare_and_sort_commands(&mut self, ctx: &mut RenderContext) {
        ...
        let mut use_transmission = false;  // todo: extract from scene
        ...
        for item_idx in 0..ctx.extracted_scene.render_items.len() {
            ...
            let (pipeline, pipeline_id) = if let Some(p) = ctx.pipeline_cache.get_pipeline_fast(fast_key) {
                ...
                let (pipeline, pipeline_id) = ctx.pipeline_cache.get_pipeline(
                    ...
                    ctx.frame_resources,    // group 3: screen_bind_group
                );
        ...
        self.commands_bundle.use_transmission = use_transmission;
...
impl RenderNode for ForwardRenderPass {
    ...
    fn run(&self, ctx: &mut RenderContext, encoder: &mut wgpu::CommandEncoder) {
        ...
        // 4. Begin render pass and execute drawing
        {
            ...
            // let (load_op, store_op) = if resolve_view.is_some() {
            //     (wgpu::LoadOp::Clear(self.clear_color), wgpu::StoreOp::Discard) 
            // } else {
            //     (wgpu::LoadOp::Clear(self.clear_color), wgpu::StoreOp::Store)
            // };
            ...
            // Opaque Pass
            {
                let opaque_pass = encoder.begin_render_pass(&pass_desc);
                let mut tracked_opaque_pass = TrackedRenderPass::new(opaque_pass);

                if let Some(gpu_global_bind_group) = &commands_bundle.gpu_global_bind_group {
                    tracked_opaque_pass.set_bind_group(0, commands_bundle.gpu_global_bind_group_id, &gpu_global_bind_group, &[]);
                } else {
                    return;
                }
                ...
                Self::draw_list(
                    ctx,
                    &mut tracked_opaque_pass,
                    &commands_bundle.opaque_commands,
                );

            }

            if commands_bundle.use_transmission {
                if ctx.wgpu_ctx.enable_hdr {
                    if let Some(transmission_view) = &ctx.frame_resources.transmission_view {
                        // 执行拷贝
                    ...
                    log::warn!("Transmission is enabled but HDR mode is off. Transmission requires HDR rendering.");
                }
            }



            let (store_op, final_resolve_target) = if resolve_view.is_some() {
                 // 最终输出，如果有 MSAA，现在 Resolve，且不需要保存 MSAA buffer 了
                (wgpu::StoreOp::Discard, resolve_view)
            } else {
            ...
            // 透明物体 Pass
            let transparent_pass = encoder.begin_render_pass(&pass_desc);
            let mut tracked_transparent_pass = TrackedRenderPass::new(transparent_pass);

            if let Some(gpu_global_bind_group) = &commands_bundle.gpu_global_bind_group {
                tracked_transparent_pass.set_bind_group(0, commands_bundle.gpu_global_bind_group_id, &gpu_global_bind_group, &[]);
            } else {
                return;
            }
