...
fn get_asset_runtime() -> &'static Runtime {
    ...
    RUNTIME.get_or_init(|| {
        Runtime::new().expect("Failed to create asset loader runtime")
    })
}
...
impl AssetServer {
    ...
    pub fn load_texture(&mut self, source: impl AssetSource, color_space: ColorSpace, generate_mipmaps: bool) -> anyhow::Result<TextureHandle> {
        #[cfg(not(target_arch = "wasm32"))]
        {
            get_asset_runtime().block_on(self.load_texture_async(source, color_space, generate_mipmaps))
        }
        #[cfg(target_arch = "wasm32")]
        {
            let mut texture = crate::assets::load_texture_from_file(source.uri().to_string(), color_space)?;
    ...
    pub fn load_cube_texture(&mut self, sources: [impl AssetSource; 6], color_space: ColorSpace, generate_mipmaps: bool) -> anyhow::Result<TextureHandle> {
        #[cfg(not(target_arch = "wasm32"))]
        {
            get_asset_runtime().block_on(self.load_cube_texture_async(sources, color_space, generate_mipmaps))
        }
        #[cfg(target_arch = "wasm32")]
        {
            let mut texture = crate::assets::load_cube_texture_from_files(sources.map(|s| s.uri().to_string()), color_space)?;
            texture.generate_mipmaps = generate_mipmaps;
            let handle = self.textures.add(texture);
            Ok(handle)
            // panic!("Synchronous loading not supported on WASM");
    ...
    pub async fn load_texture_async(&self, source: impl AssetSource, color_space: crate::assets::ColorSpace, generate_mipmaps: bool) -> anyhow::Result<TextureHandle> {
    ...
    pub async fn load_cube_texture_async(&self, sources: [impl AssetSource; 6], color_space: ColorSpace, generate_mipmaps: bool) -> anyhow::Result<TextureHandle> {
        // 并发加载 6 张图
        ...
        if images.iter().any(|img| img.width() != width || img.height() != height) {
    ...
    /// 异步加载 HDR 环境贴图
    pub async fn load_hdr_texture_async(&self, source: impl AssetSource) -> anyhow::Result<TextureHandle> {
        let reader = AssetReaderVariant::new(&source)?;
        let filename = source.filename().unwrap_or(std::borrow::Cow::Borrowed("unknown"));
    ...
    async fn decode_image_async(bytes: Vec<u8>, color_space: ColorSpace, label: String) -> anyhow::Result<crate::resources::image::Image> {
        #[cfg(not(target_arch = "wasm32"))]
        {
            // Native: 放到 blocking thread 执行
            tokio::task::spawn_blocking(move || Self::decode_image_cpu(bytes, color_space, label)).await?
        }
        #[cfg(target_arch = "wasm32")]
        {
    ...
    /// CPU 图片解码逻辑
    fn decode_image_cpu(bytes: Vec<u8>, color_space: ColorSpace, label: String) -> anyhow::Result<crate::resources::image::Image> {
