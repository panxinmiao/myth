...
@group(0) @binding(1)
var<storage, read> splat_appearance: array<SplatAppearance>;
@group(0) @binding(2)
...
fn vs_main(@builtin(vertex_index) vertex_idx: u32, @builtin(instance_index) instance_idx: u32) -> VertexOutput {
    ...
    let splat_geom = splat_geometry[sorted_idx];
    let splat_attr = splat_appearance[sorted_idx];

    let center_ndc = splat_geom.pos;
    let axis_0 = unpack2x16float(splat_geom.v_0);
    let axis_1 = unpack2x16float(splat_geom.v_1);
    let color_rg = unpack2x16float(splat_attr.color_0);
    let color_ba = unpack2x16float(splat_attr.color_1);
