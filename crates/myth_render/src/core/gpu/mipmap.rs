...
/// A persistent texture whose mip chain must be generated by the RDG.
///
/// The request owns a cheap clone of the WGPU texture handle and an atomic
/// completion token shared with the owning `GpuImage`. The pass node only
/// borrows a slice of requests, keeping RDG nodes trivially droppable.
pub(crate) struct MipmapRequest {
...
impl MipmapRequest {
    ...
    pub(crate) const fn key(&self) -> (u64, u64) {
        (self.texture_id, self.generation_id)
    }
